...
# ---------------------------------------------------------------------------
#
# Every check used to be invoked bare at the bottom of this file, under the
# script-wide `set -e`. One failure therefore aborted the run, and the ~240
# checks queued behind it never executed -- so a single broken assertion hid
# the state of the entire suite. (That is exactly how ~240 assertions stayed
# masked behind one over-broad CI guard: nobody could see they had gone stale,
# because nothing after the guard ever ran.)
#
# `run_check` runs each check to completion, records the failures, and lets the
# suite continue. `smoke_report` prints the roll-up and sets the exit status.
SMOKE_TOTAL=0
SMOKE_FAILED=()

# Run one check, surviving its failure.
#
# The subshell is what keeps failures non-fatal to the SUITE while `set -e`
# stays active inside it, so an unexpected command failure part-way through a
# check is still fatal to THAT CHECK. Both obvious alternatives lose the second
# half of that:
#
#   if ! "$name"; then ...            # condition context
#   set +e; "$name"; rc=$?; set -e    # errexit is global shell state
#
# bash disables errexit for the whole call tree in either case, so a check whose
# middle command failed would keep running past the failure and could still
# `return 0` -- a vacuous pass, silently converting real failures into green.
# Clearing errexit in the PARENT and re-arming it inside the subshell is the
# spelling that does not leak: the subshell is a plain command, not an operand
# of `!`/`&&`/`||`, so its own `set -e` takes effect.
#
# Verified against `chk() { false; echo reached; return 0; }`: both spellings
# above print "reached" and report success; this one reports 1 and prints
# nothing. Any future edit here must preserve that property.
#
# (A subshell does not inherit the EXIT trap, so `cleanup_smoke_tmp` does not
# fire per check and SMOKE_TMP_BASE survives the whole run.)
run_check() {
  local name="$1"
  shift
  local rc=0
  ...
  if ((rc != 0)); then
    printf '[lvim-smoke] FAIL (rc=%d): %s\n' "$rc" "$name" >&2
    SMOKE_FAILED+=("$name")
  fi
}

# Same contract for a labelled external command (`make test`, the integration
# smoke) rather than a check function, so a failing plenary run no longer stops
# the integration smoke from running -- and vice versa.
run_step() {
  local label="$1"
  shift
  local rc=0
...
smoke_report() {
  if ((${#SMOKE_FAILED[@]} == 0)); then
    printf '[lvim-smoke] all %d checks passed\n' "$SMOKE_TOTAL"
    return 0
  fi

  local name
  printf '\n[lvim-smoke] %d of %d checks FAILED:\n' \
    "${#SMOKE_FAILED[@]}" "$SMOKE_TOTAL" >&2
  for name in "${SMOKE_FAILED[@]}"; do
    printf '  - %s\n' "$name" >&2
  done
  return 1
}

# Bail out on a broken precondition -- but not before saying what the checks
# that already ran found. Suppressing that roll-up is the exact failure mode
# `run_check` exists to eliminate, and a hard abort is no reason to reintroduce
# it: the preconditions below sit partway down the run, not at the top.
smoke_abort() {
  smoke_report || true
  printf '%s\n' "$1" >&2
  exit 1
...
# These two guards abort rather than collect, deliberately -- they are
# preconditions, not assertions. Almost every check below boots
# `nvim -u init.lua` or shells out to `bin/lvim`; with either missing, ~240
# checks would each spend a Neovim launch to report the same single fact.
# `smoke_abort` still prints the roll-up for whatever ran first.
if [[ ! -f init.lua ]]; then
  smoke_abort 'init.lua is missing at repository root'
fi

run_check check_phase_93_scanner_lexing
run_check check_phase_93_no_runtime_references_to_reference_tree

run_check check_nvim_init init.lua
run_check check_globals_present
run_check check_min_nvim_version_error

if [[ ! -x bin/lvim ]]; then
  smoke_abort 'bin/lvim is missing or not executable'
fi

run_check check_lvim_launcher
run_check check_lvim_appname
run_check check_lvim_launcher_uses_repo_init
run_check check_user_config_applied
run_check check_user_config_literal_acceptance
run_check check_builtin_deep_merge_semantics
run_check check_merge_builtin_overrides_api
run_check check_merge_builtin_overrides_type_guards
run_check check_deep_extend_force_keep_user_helper
run_check check_missing_config_hint
run_check check_user_plugins_appended
run_check check_final_spec_filters_disabled_builtin
run_check check_disabled_builtin_filter_is_core_only
run_check check_final_spec_order_core_before_user
run_check check_anonymous_core_entry_passes_through
run_check check_multiple_disabled_builtins_all_filtered
run_check check_reload_globals
run_check check_lazy_bootstrap_idempotent
run_check check_phase_22_plugin_count
run_check check_phase_22_load_triggers
run_check check_phase_23_commands_registered
run_check check_phase_23_lvim_info_renders
run_check check_phase_23_lvim_cache_reset_clears_dir
...
run_check check_phase_25_second_launch_idempotency
run_check check_phase_31_options_defaults_applied
run_check check_phase_31_user_opt_override_wrap
run_check check_phase_32_acceptance_commands_literal
...
run_check check_phase_33_dir_opened_re_emits_originating_event
...
run_check check_phase_34_lvim_reload_emits_notify
run_check check_phase_34_lvim_reload_literal_acceptance
...
run_check check_phase_42_defaults_attached_to_each_server
run_check check_phase_42_user_on_attach_overrides_default
run_check check_phase_42_default_on_attach_registers_keymaps
run_check check_phase_42_capabilities_baseline
run_check check_phase_42_user_capabilities_overrides_default
run_check check_phase_42_per_server_on_attach_overrides_global
run_check check_phase_42_multiple_servers_all_setup
run_check check_phase_42_empty_servers_no_setup_calls
...
run_check check_phase_43_reload_idempotent
run_check check_phase_43_exclude_clients_filter_drops_named
run_check check_phase_43_user_filter_overrides_exclude_clients
run_check check_phase_43_table_without_enabled_is_disabled
...
run_check check_phase_45_signs_render_with_prescribed_glyph
run_check check_phase_45_signs_numhl_render_with_prescribed_highlight
...
run_check check_phase_51_treesitter_setup_pcall_guards_missing
run_check check_phase_51_treesitter_toggle_drops_plugin
run_check check_phase_51_literal_require_nvim_treesitter
...
run_check check_phase_52_setup_pcall_guards_missing
run_check check_phase_52_pre_hook_sets_jsx_commentstring_for_tsx_ft
run_check check_phase_52_pre_hook_leaves_non_jsx_buffer_alone
run_check check_phase_52_pre_hook_prefers_treesitter_node_when_available
run_check check_phase_52_pre_hook_walks_parent_chain_for_jsx
run_check check_phase_52_pre_hook_uses_ref_position_not_cursor
...
run_check check_phase_6_telescope_setup_pcall_guards_missing
run_check check_phase_6_telescope_leader_f_group_maps_registered
run_check check_phase_6_telescope_user_override_forwarded
run_check check_phase_6_telescope_setup_does_not_mutate_builtin
run_check check_phase_6_telescope_literal_acceptance_telescope_find_files
run_check check_phase_6_telescope_literal_acceptance_require_returns_table
run_check check_phase_6_telescope_toggle_drops_telescope_specifically
run_check check_phase_6_nvimtree_module_present
run_check check_phase_6_nvimtree_defaults_shape
run_check check_phase_6_nvimtree_lvimexplorer_focuses_new_sidebar
run_check check_phase_6_nvimtree_on_attach_cr_passes_node
run_check check_phase_6_nvimtree_setup_disables_netrw
run_check check_phase_6_nvimtree_setup_forwards_opts
run_check check_phase_6_nvimtree_setup_pcall_guards_missing
run_check check_phase_6_nvimtree_leader_e_map_registered
run_check check_phase_6_nvimtree_toggle_does_not_error
run_check check_phase_6_nvimtree_user_override_forwarded
...
run_check check_phase_6_lualine_setup_pcall_guards_missing
run_check check_phase_6_lualine_user_override_forwarded
run_check check_phase_6_lualine_setup_does_not_mutate_builtin
...
run_check check_phase_6_bufferline_setup_pcall_guards_missing
run_check check_phase_6_bufferline_user_override_forwarded
run_check check_phase_6_bufferline_setup_does_not_mutate_builtin
...
run_check check_phase_6_gitsigns_setup_pcall_guards_missing
run_check check_phase_6_gitsigns_user_override_forwarded
run_check check_phase_6_gitsigns_setup_does_not_mutate_builtin
run_check check_phase_6_gitsigns_leader_g_group_maps_registered
...
run_check check_phase_6_whichkey_setup_pcall_guards_missing
run_check check_phase_6_whichkey_user_override_forwarded
run_check check_phase_6_whichkey_setup_does_not_mutate_builtin
...
run_check check_phase_6_terminal_user_override_forwarded
run_check check_phase_6_terminal_setup_does_not_mutate_builtin
run_check check_phase_6_terminal_lazygit_gated_on_executable
run_check check_phase_6_terminal_toggle_lazygit_caches_terminal
run_check check_phase_6_terminal_toggle_lazygit_pcall_guards_missing
run_check check_phase_6_terminal_literal_acceptance_require_returns_table
run_check check_phase_6_terminal_toggle_drops_terminal_specifically
run_check check_phase_6_terminal_lazygit_map_rhs_calls_toggle_lazygit
run_check check_phase_6_terminal_lazygit_recipe_opts_full_shape
run_check check_phase_6_breadcrumbs_module_present
run_check check_phase_6_breadcrumbs_defaults_shape
run_check check_phase_6_breadcrumbs_setup_forwards_opts
run_check check_phase_6_breadcrumbs_setup_pcall_guards_missing
run_check check_phase_6_breadcrumbs_setup_arms_winbar
run_check check_phase_6_breadcrumbs_setup_does_not_mutate_builtin
run_check check_phase_6_breadcrumbs_user_override_forwarded
run_check check_phase_6_breadcrumbs_on_attach_calls_navic_attach
run_check check_phase_6_breadcrumbs_on_attach_skips_without_capability
run_check check_phase_6_breadcrumbs_on_attach_skips_when_disabled
run_check check_phase_6_breadcrumbs_spec_uses_breadcrumbs_active_gate
...
run_check check_phase_6_indentlines_setup_does_not_mutate_builtin
run_check check_phase_6_indentlines_user_override_forwarded
run_check check_phase_6_indentlines_spec_uses_indentlines_active_gate
run_check check_phase_6_indentlines_literal_acceptance_require_returns_table
...
if [[ -f tests/minimal_init.lua ]]; then
  run_check check_nvim_init tests/minimal_init.lua
fi
...
if [[ "${SKIP_INTEGRATION:-0}" == "1" ]]; then
  ...
  run_step "scripts/integration-smoke.sh" bash scripts/integration-smoke.sh
fi
